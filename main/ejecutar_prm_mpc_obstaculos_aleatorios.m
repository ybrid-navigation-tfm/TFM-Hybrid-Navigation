%% PRM + MPC en un entorno dinamico con obstaculos aleatorios
% Ensambla los modulos del proyecto TFM_RobotNavigation para ejecutar una
% simulacion completa de la arquitectura PRM + MPC.
%
% La roadmap PRM persistente se construye exclusivamente con la geometria
% estatica y utiliza una semilla fija del planificador al comienzo de cada
% ejecucion. Esta configuracion no se modifica durante la simulacion.
%
% Los obstaculos moviles utilizan un RandStream independiente, de modo que
% sus posiciones iniciales y sus rebotes no alteran el generador
% pseudoaleatorio empleado por PRM.
%
% MPC recibe el objetivo adelantado comun, predice el movimiento del robot
% y de los obstaculos y genera el control [v w] para el mismo modelo de
% uniciclo utilizado en las otras arquitecturas.
clearvars;
clc;
close all;
...
% Semilla fija del planificador. PRM comienza todas las ejecuciones desde
...
%   rebotes sin modificar el generador utilizado por PRM.
%
% "reproducible":
...
% antes de construir la roadmap PRM.
...
% pseudoaleatoria reservada para PRM.
...
[obstaculosDinamicosIniciales,infoInicializacionObstaculos] = ...
    inicializar_obstaculos_aleatorios( ...
        escenario,robot,cfg,flujoObstaculos);

% PRM sigue construyendo su roadmap solo con la geometria estatica.
escenario.obstaculosDinamicos = obstaculosDinamicosIniciales;
...
if exportarAnimacion
    ...
    nombreAnimacion = "prm_mpc_aleatorio_"+string(escenario.id)+ ...
    ...
    opcionesExportacion.factorVelocidad = factorVelocidadAnimacion;
...
%% Visualizacion y simulacion PRM + MPC
% La figura se crea directamente en el Live Script, igual que en las
% versiones aleatorias RRT*+APF y RRT*+MPC.
if cfg.visual.activa
...
graficos = inicializar_figura( ...
    escenario,cfg,cfg.nombreArquitectura,figuraLive);
...
if exportarAnimacion && graficos.activa
    [exportacion,infoExportacion] = exportar_animacion( ...
        "iniciar",[],graficos,cfg,archivoAnimacion,opcionesExportacion);
...
% -------------------------------------------------------------------------
        ...
        numeroRebotesAleatorios = numeroRebotesAleatorios + ...
...
% -------------------------------------------------------------------------
        if graficos.activa
            ...
            tiempoVisualizacion(k) = toc(relojVisualizacion);
...
if guardarResultado
    ...
    nombreBase = "prm_mpc_aleatorio_"+string(escenario.id)+ ...
        "_semilla_"+string(cfg.semilla)+ ...
...
disp(" ");
disp("Resumen de la ejecucion PRM + MPC con obstaculos aleatorios");
disp(filaResultado);
